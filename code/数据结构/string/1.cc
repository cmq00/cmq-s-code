...
{ // 返回子串T在主串S中第pos个字符之后第一次出现的位置。若不存在,则函数值为0。 
...
int Index1(SString S, SString T, int pos) {  // 算法4.1 
...
do 
{ 
k=Index1(S, T, i); // 结果i为从上一个i之后找到的子串T的位置 
if (k) // 串S中存在串T 
{ 
StrDelete(S, k, StrLength(T)); // 删除该串T 
StrInsert(S, k, V); // 在原串T的位置插入串V 
